...
def _load_cluster_indexes(prefix: Path) -> Dict[str, Dict[str, Any]]:
    ...
    for chunks_file in cluster_dir.glob("*_chunks.json"):
        cluster_name = chunks_file.name[: -len("_chunks.json")]
        index_file = cluster_dir / f"{cluster_name}_faiss.index"
        corpus_file = cluster_dir / f"{cluster_name}_bm25_corpus.pkl"
